...
Bạn có thể chạy công cụ thông qua lệnh `uv run` đối với script `ohash.py` (hoặc `uv run src/cli.py`), hoặc chạy tệp thư mục gốc nếu dùng `setup.sh`.

### 1. Mã hóa tập tin (Encrypt)

```bash
# Sử dụng Block Mode (Mặc định)
uv run ohash.py e <file_cần_mã_hóa> [file_đầu_ra]
uv run src/cli.py e <file_cần_mã_hóa>

# Sử dụng Chunked Mode 
uv run ohash.py e <file_cần_mã_hóa> --cnk [file_đầu_ra]
```
...
```bash
uv run ohash.py d <file_đã_mã_hóa> [file_đầu_ra]
uv run src/cli.py d <file_đã_mã_hóa>
```
