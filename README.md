...
- Python 3.x
- Thư viện `cryptography` (Cài đặt qua pip: `pip install cryptography`)
...
Bạn có thể chạy công cụ thông qua script `ohash.py` (hoặc `python src/cli.py`), hoặc nếu đã chạy `setup.sh` để cài đặt lệnh global `ohash`.

### 1. Mã hóa tập tin (Encrypt)

```bash
# Sử dụng Block Mode (Mặc định)
python ohash.py e <file_cần_mã_hóa> [file_đầu_ra]
python src/cli.py e <file_cần_mã_hóa>

# Sử dụng Chunked Mode 
python ohash.py e <file_cần_mã_hóa> --cnk [file_đầu_ra]
```
...
```bash
python ohash.py d <file_đã_mã_hóa> [file_đầu_ra]
python src/cli.py d <file_đã_mã_hóa>
```
