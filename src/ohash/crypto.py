...
from .helpers import (
    derive_key, create_header, parse_header, HEADER_SIZE
)
...
class Encrypter:
    ...
    def _encrypt_block_mode(self, input_path: Path, output_path: Path, file_size: int,
        ...
        header = create_header(self._salt, ROUNDS, file_size, self._master_nonce, EncMode.BCK)
    ...
    def _encrypt_chunk_mode(self, input_path: Path, output_path: Path, file_size: int,
        ...
        # Write header first
        header = create_header(self._salt, ROUNDS, file_size, self._master_nonce, EncMode.CNK)
...
class Decrypter:
    ...
    def read_header(self, input_path: Path) -> None:
        ...
        salt, rounds, file_size, nonce, mode = parse_header(header_bytes)

        # Derive key
        self._key = derive_key(self._passphrase, salt, rounds)
